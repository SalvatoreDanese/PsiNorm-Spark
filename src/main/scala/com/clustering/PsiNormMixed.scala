...
object PsiNormMixed {
  ...
  def main(args: Array[String]): Unit = {
    ...
    var matrix: RowMatrix = new RowMatrix(parsedData)

    matrix = multiplyColumns(matrix, sums, rowsInt)
    matrix.rows.foreach(v => println(v))

    }

  def multiplyColumns(m: RowMatrix, xs: Array[Double], rowsInt: Int): RowMatrix = {
    val newRowsRdd = m.rows.map {
      row => Vectors.dense(row.toArray.zip(xs).map{case (a, b) => a * rowsInt/b})
    }
